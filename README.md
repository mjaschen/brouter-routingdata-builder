...
SRTM data with 3 arc seconds resolution (~90 m) can be downloaded here: <http://srtm.csi.cgiar.org/srtmdata/>
...
It's also possible to download the whole set at this [Google Drive Folder](https://drive.google.com/folderview?id=0B_J08t5spvd8RWRmYmtFa2puZEE&usp=drive_web#list). Unpack the archive and mount the directory `SRTMv4.1/5_5x5_ascii` into the container.

### High Resolution SRTM Data (optional)

[Sonny](https://sonny.4lima.de) provides curated high resolution (1 arc second) SRTM data for Europe in HGT format.

These files can be converted to BEF format using the `ElevationRasterTileConverter` tool which is part of BRouter.
...
docker run --rm \
    ...
    --volume /tank/brouter/srtm1:/srtm1:ro \
    --volume /tank/brouter/srtm3:/srtm3:ro \
...
- `--volume /tank/brouter/srtm1:/srtm1:ro`: the directory where the high resolution (1 arc second) SRTM data is located, in BEF (`*.bef`) format; it's mounted read-only as there are no changes applied to the SRTM data. If no high resolution SRTM data is available, just omit this option.
- `--volume /tank/brouter/srtm3:/srtm3:ro`: the directory where the default (3 arc seconds resolution) SRTM data is located, either in zipped ASCII format (`*.zip`) or in BEF (`*.bef`) format; it's mounted read-only as there are no changes applied to the SRTM data
